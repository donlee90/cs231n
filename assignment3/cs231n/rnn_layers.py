...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_t = dh[:,t,:] + dprev_h
        dx, dprev_h, dWx, dWh, db = rnn_step_backward(dh_t, cache[t])
    ...
    dx = np.stack(reversed(dxs), axis=1)
    dWx = np.sum(dWxs, axis=0)
    dWh = np.sum(dWhs, axis=0)
    db = np.sum(dbs, axis=0)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    do_hat = do * o * (1 - o)
    dg_hat = dg * (1 - g ** 2) 
    da = np.hstack([di_hat, df_hat, do_hat, dg_hat])
...
def lstm_backward(dh, cache):
    ...
    dprev_h = np.zeros(dh[:,0,:].shape)
    dprev_c = np.zeros(dh[:,0,:].shape)
    for t in reversed(range(T)):
        dh_t = dh[:,t,:] + dprev_h
        dx, dprev_h, dprev_c, dWx, dWh, db = lstm_step_backward(dh_t, dprev_c, cache[t])
