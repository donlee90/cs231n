...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for l in range(self.num_layers):
            if l == 0:
                dim_in = input_dim
                dim_out = hidden_dims[l]
            elif l == self.num_layers - 1:
                dim_in = hidden_dims[l-1]
                dim_out = num_classes
            else:
                dim_in = hidden_dims[l-1]
                dim_out = hidden_dims[l]

            self.params['W%d'%(l+1)] = np.random.normal(0, weight_scale,
                                                        (dim_in, dim_out))
            self.params['b%d'%(l+1)] = np.zeros(dim_out)
             #print 'W%d' % (l+1), 'b%d' % (l+1)

            # Initialize BatchNorm layers
            if self.use_batchnorm:
                if l < self.num_layers - 1:
                    self.params['gamma%d'%(l+1)] = np.ones(dim_out)
                    self.params['beta%d'%(l+1)] = np.zeros(dim_out)
    ...
    def loss(self, X, y=None):
        ...
        for l in range(self.num_layers):
            ...
            if l < self.num_layers - 1:
                ...
                # BatchNorm layer forward
                if self.use_batchnorm:
                    gamma = self.params['gamma%d'%(l+1)]
                    beta = self.params['beta%d'%(l+1)]
                    layer_in, cache_bn = batchnorm_forward(layer_in,
                            gamma, beta, self.bn_params[l])

                # ReLu layer forward
                layer_in, cache_relu = relu_forward(layer_in)

                # Dropout layer forward
                if self.use_dropout:
                    layer_in, cache_dropout = dropout_forward(layer_in,
                            self.dropout_param)
                caches.append((cache,cache_bn,cache_relu,cache_dropout))
            else:
                caches.append((cache,))
        ...
        for l in reversed(range(self.num_layers)):
            ...
            if l == self.num_layers - 1:
                dx, dW, db = affine_backward(dL, caches[l][0])
            else:
                cache, cache_bn, cache_relu, cache_dropout = caches[l]
                if self.use_dropout:
